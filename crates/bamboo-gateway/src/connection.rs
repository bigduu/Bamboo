...
impl ConnectionPool {
    ...
    pub async fn send_to(&self, connection_id: &str, event: GatewayEvent) -> Result<(), ConnectionError> {
        let connections = self.connections.lock().await;
                    ...
                    Some(msg) = ws_receiver.next() => {
                        match msg {
                            Ok(Message::Text(text)) => {
                                ...
                                if let Ok(event) = serde_json::from_str::<crate::protocol::ClientMessage>(&text) {
                                    if matches!(event, crate::protocol::ClientMessage::Ping { .. }) {
